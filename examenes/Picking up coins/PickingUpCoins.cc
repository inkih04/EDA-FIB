...
int dfs(int n, int m, const Point& p, Tablero& map, vector<vector<bool>>& marked, const int N_DIRS, const int DR[], const int DC[]) {
    int s = 0;
    marked[p.r][p.c] = true;

    for (int i = 0; i < N_DIRS; ++i) {
        Point q(p.r + DR[i], p.c + DC[i]);
        if (ok(n, m, q) and map[q.r][q.c] != 'T' and !marked[q.r][q.c])
